...
#models
from models.baseline_METNETV2 import MetNet2 as Base_METNET2

VERBOSE = False
#VERBOSE = True

class MetNet2_Lightning(pl.LightningModule):
    def __init__(self, UNet_params: dict, params: dict,
                 **kwargs):
        super(MetNet2_Lightning, self).__init__()

        self.input_channels = params['in_channels']
        self.model = Base_METNET2(in_channels=self.input_channels)
    ...
    def forward(self, x):
        out = [self.model(x, lead_time) for lead_time in range(32)]
        out = torch.stack(out, dim=1)
        
        outs = out.shape
        out = torch.reshape(out, (outs[0], outs[2], outs[1], outs[3], outs[4]))
